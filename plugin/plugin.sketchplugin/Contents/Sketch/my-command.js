...
exports['default'] = function (context) {
    ...
    context.document.pages().forEach(function (page, i) {
        page.layers().forEach(function (layer, i) {
            layer.style().fills().forEach(function (attr, i) {
                colors.push(attr.color().RGBADictionary());
            });

            layer.style().borders().forEach(function (attr, i) {
                colors.push(attr.color().RGBADictionary());
            });

            layer.style().shadows().forEach(function (attr, i) {
                colors.push(attr.color().RGBADictionary());
            });

            layer.style().innerShadows().forEach(function (attr, i) {
                colors.push(attr.color().RGBADictionary());
            });
        });
    });

    colors = colors.map(function (_ref) {
        var r = _ref.r,
            g = _ref.g,
            b = _ref.b,
            a = _ref.a;
        return 'rgba(' + String(_c(r)) + ', ' + String(_c(g)) + ', ' + String(_c(b)) + ', ' + String(a) + ')';
    }).filter(_unique);

    fetch('https://secure-woodland-80317.herokuapp.com/', { method: 'POST', body: JSON.stringify(colors) }).then(function (res) {
        return res.text();
    }).then(function (text) {
        return log(text);
    })['catch'](function (text) {
